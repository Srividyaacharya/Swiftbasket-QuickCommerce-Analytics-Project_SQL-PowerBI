...
- Audit and clean 450K+ rows of messy raw data using a structured Bronze → Silver → Gold architecture
...
- Calculated ROI = gross margin earned ÷ discount given
...
- Over 80 products classified as Stockout or Critical (≤2 days remaining) — this is systemic, not isolated
...
- ₹17.3L stockout leakage at 4.8% stockout rate — nearly ₹1 in every ₹20 of potential revenue is being lost to unavailability
...
| Overview | ₹17.3L lost to stockouts (4.8% rate) | Recoverable with better inventory planning |
...
| Promotions | Struggling stores discount 3.5x more than Star | Promos can't fix broken ops |
...
**Inventory & Stockout**
- Implement daily replenishment for perishables in Struggling stores — Milk, Eggs, Butter, Chicken account for the majority of the ₹17.3L stockout loss
...
| **Schema Architecture** | Implemented Bronze → Silver → Gold medallion architecture using dual `raw.*` and `clean.*` schemas |
...
🔗 [Credit Risk Analysis Project](../credit-risk-analysis)
